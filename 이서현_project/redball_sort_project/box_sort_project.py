...
GRIPPER_OPEN = 0.019
GRIPPER_CLOSE = -0.01
ANGULAR_SPEED_LIMIT_RAD_S = 0.15  # 관절이 이동할 때 허용하는 최대 각속도(라디안/초).
                          # 실제로 겪은 문제: 이동 시간을 거리와 상관없이 고정값으로 두면, 자리가
                          # 늘어나서(4개, 최대 180도 회전) 오히려 각속도가 더 빨라져 박스를 쥔 채로
                          # 빠르게 돌다가 박스가 튕겨나감(실측: 목표 자리와 전혀 다른 곳에 떨어짐).
...
OCCUPANCY_MATCH_DIST_M = 0.1  # 마커의 world (x,y)가 어떤 자리 위치와 이 거리(m) 안이면 "그 자리에 있다"고 판단.
                          # 자리 사이 간격(90도, 반지름 0.283m 기준 약 44cm 호)보다 훨씬 작게 잡아서
                          # 옆 자리와 헷갈리지 않게 함.
...
MOVE_TIMEOUT_SEC = 35.0   # COMMAND 실행 중 "팔이 도착할 때까지" 기다리는 최대 시간 (더 걸리면 포기).
...
# ---- 아루코 인식용 상수 ----
ARUCO_DICT = cv2.aruco.Dictionary_get(cv2.aruco.DICT_4X4_50)  # aruco_cube 모델이 쓰는 것과 같은 사전
...
class BoxSortProject(Node):
    ...
    def image_callback(self, msg: Image):
        ...
        corners, ids, _rejected = cv2.aruco.detectMarkers(gray, ARUCO_DICT, parameters=ARUCO_PARAMS)
    ...
    # ------------------------------------------------------------------
    def run_command(self, marker_id: int, to_slot: int):
        if to_slot not in self.slots:
            self.get_logger().info(f"{to_slot}번 자리는 아직 안 가르쳤습니다.")
            return

        from_slot = self.find_slot_of_marker(marker_id)
        if from_slot is None:
            self.get_logger().info(f"마커 id={marker_id} 박스를 천장 카메라에서 못 찾았습니다.")
            return

        if from_slot == to_slot:
            self.get_logger().info(f"마커 id={marker_id} 박스는 이미 {to_slot}번 자리에 있습니다.")
            return
        ...
        if occupied_by is not None and occupied_by != marker_id:
            self.get_logger().info(
                f"{to_slot}번 자리에는 이미 마커 id={occupied_by} 박스가 있어서 이동할 수 없습니다."
            )
            return
        ...
        if not self._gripper_and_wait(GRIPPER_OPEN):
            self.get_logger().info("그리퍼를 여는 데 실패해서 중단합니다.")
            return
        if not self._safe_move_to(self.slots[from_slot]["joints"]):
            self.get_logger().info("팔이 출발 자리에 도착하지 못해서 중단합니다.")
            return

        # 2) 집기
        if not self._gripper_and_wait(GRIPPER_CLOSE):
            self.get_logger().info("그리퍼를 닫는 데 실패해서 중단합니다.")
            return

        # 3) 도착 자리로 안전하게 이동
        if not self._safe_move_to(self.slots[to_slot]["joints"]):
            self.get_logger().info("팔이 도착 자리에 도착하지 못해서 중단합니다 (그리퍼는 아직 닫힌 상태).")
            return

        # 4) 놓기
        if not self._gripper_and_wait(GRIPPER_OPEN):
            self.get_logger().info("그리퍼를 여는 데 실패했습니다 (팔은 도착 자리에 있음).")
            return

        self.get_logger().info(f"마커 id={marker_id} 박스: {from_slot}번 -> {to_slot}번 이동 완료")
    ...
    # ------------------------------------------------------------------
    def _send_trajectory_once(self, positions) -> bool:
        ...
        duration_sec = max(3.0, max_delta / ANGULAR_SPEED_LIMIT_RAD_S)
    ...
    def _move_and_wait(self, positions, max_retries=3) -> bool:
        ...
        for attempt in range(1, max_retries + 1):
            if not self._send_trajectory_once(command):
                return False
