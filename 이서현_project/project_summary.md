...
이 패키지에는 실행 파일이 2개 있습니다: **`redball_tracking_project.py`**(1단계, 빨간 공을 카메라로 쫓아가서 잡기)와 **`box_sort_project.py`**(2단계, 아루코 박스를 정해진 자리끼리 옮기기). 이 문서는 둘 다 기술적으로 자세히 다룹니다. 쉽게 풀어쓴 버전은 [project_easy.md](project_easy.md), 실행 명령어만 보려면 [run_guide.md](run_guide.md), 파일별 수정법은 [customize_guide.md](customize_guide.md)를 보세요.
...
└── ros_ws/src/
    ...
    │   ├── project_summary.md                  지금 보고 있는 이 설명 파일
    │   ├── project_easy.md                     쉽게 풀어쓴 버전
...
**아직 완전히 해결 안 된 부분**: 재시도-보정 방식은 **빈 공간으로 이동할 때는** 정확히 도착하지만, **실제 박스가 목표 지점에 있는 상태**에서는 접근 경로 중간에 팔/그리퍼가 박스와 부딪히면서 재시도해도 수렴이 잘 안 되는 걸 확인했습니다. 이건 "단순히 관절 각도만 맞추면 되는 문제"가 아니라 "경로 중 충돌을 피하면서 계획해야 하는 문제"라, 다음 단계로 **MoveIt 기반 충돌회피 경로 계획**을 시도했습니다 (아래).
