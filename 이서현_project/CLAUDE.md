...
- [project_summary.md](project_summary.md) — 기술적으로 자세한 설명, 겪은 버그와 해결 과정 전부 기록됨
- [project_easy.md](project_easy.md) — 같은 내용을 쉽게 풀어쓴 버전
- [run_guide.md](run_guide.md) — 실행 명령어만
- [customize_guide.md](customize_guide.md) — 파일별로 뭘 어떻게 고칠 수 있는지

**새 세션에서 이 프로젝트를 이어서 작업한다면, 위 문서들 특히 project_summary.md의 "오늘 찾은 버그" 섹션들을 먼저 읽고 시작하세요. 이미 겪고 고친 문제를 반복하지 않기 위함입니다.**
...
- **`open_manipulator_moveit_config`(open_manipulator_ws) 는 이미 built/installed 되어 있고 이 로봇에 맞음.** SRDF: `arm`/`gripper` 그룹, `init`/`home`/`open`/`close`/`sector_030~330` named states 있음. 새로 만들 필요 없이 재사용.
- **URDF에 링크를 추가하면 SRDF의 `<disable_collisions>`도 같이 갱신해야 함.** camera_link를 URDF에 추가했을 때 SRDF를 안 고쳐서 MoveIt이 시작 자세부터 "충돌 중"으로 오판해 아무 계획도 못 세웠던 적이 있음.
- **MoveItPy를 `ros2 run`으로 그냥 실행하면 안 됨.** SRDF/kinematics/ompl 파라미터를 직접 못 찾아서 "Failed to load planning pipelines" 에러로 죽음. `launch/box_sort_moveit.launch.py`처럼 파라미터를 읽어서 넘겨주는 launch 파일을 거쳐야 함.
- **그리퍼의 `"close"` named state(-0.01)는 "손가락 사이에 아무것도 없을 때"를 가정한 값.** 폭이 있는 물체를 잡을 땐 이 값까지 닫으려는 계획 자체가 충돌로 거부됨 — 물체 폭에 맞는 값을 따로 찾아야 함(`grip_object()`처럼 점점 좁은 값을 시도).
- **`RRTConnect`는 확률적 플래너라 `planning_attempts`가 낮으면(기본 1) 가끔 실패함.** 빡빡한 목표일수록 여러 번 재시도하게 설정 필요(10 정도로 늘림).
...
- `box_sort_moveit_project.py` + `launch/box_sort_moveit.launch.py`: MoveIt(MoveItPy) 기반, 좌표(x,y,z)만 주면 역기구학+충돌회피 경로를 계산. 파라미터 로딩/계획/실행/충돌검사는 정상 동작 확인.
